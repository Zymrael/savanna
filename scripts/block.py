...
class TestBlock(nn.Module):
    def __init__(self, hidden_size, use_te_linear=True, use_norm=True, dtype=torch.bfloat16, is_first_block=True, block_idx=0):
        super().__init__()
...
class SplitGPUModel(nn.Module):
    def __init__(self, hidden_size=32, use_te_linear=True, use_norm=True, dtype=torch.bfloat16, num_blocks=10):
        super().__init__()
...
def create_checkpoint(hidden_size, dtype, use_norm=True):
    checkpoint = {}
...
def run_test(num_steps=3, use_norm=True):
    ...
    print("\nCreating models...")
    te_model = SplitGPUModel(use_te_linear=True, use_norm=use_norm, dtype=dtype)
    nn_model = SplitGPUModel(use_te_linear=False, use_norm=use_norm, dtype=dtype)
    
    print("\nLoading checkpoint...")
    checkpoint = create_checkpoint(hidden_size, dtype, use_norm=use_norm)
...
if __name__ == "__main__":
    ...
    run_test(use_norm=args.use_norm)
