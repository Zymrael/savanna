...
from savanna.model.transformer import (
...
from savanna.model.gmlp import GMLPBlock
...
class GPT2ModelPipe(PipelineModule, torch.nn.Module):
    ...
    :param global_config: NeoX arguments object (configuration)
    ...
    def __init__(
        ...
        super().__init__(
            layers=self.specs,
            loss_fn=partial(
                cross_entropy, _fp16=self.global_config.fp16_lm_cross_entropy
            ),
            topology=topology,
            activation_checkpoint_interval=self.global_config.checkpoint_num_layers
            if self.global_config.checkpoint_activations
            else 0,
    ...
    def init_specs(self):
        ...
        for i in range(self.global_config.num_layers):
            layer_type = self.global_config.attention_config[i]
            if layer_type in ["gmlp", "amlp"]:
                ...
                self.specs.append(
                    LayerSpec(
                        ...
                        rotary=self.global_config.pos_emb == "rotary",
        ...
        self.specs.append(
            LayerSpec(NormPipe, norm, self.global_config.hidden_size, eps=eps)
        )
