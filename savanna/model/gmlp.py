...
from savanna.model.norms import get_norm
from savanna.model.utils import get_fusion_type

from savanna import mpu


class TinyAttention(nn.Module):
    def __init__(self, global_config, d_attn, d_ff, mask_fn):
        super().__init__()
        ...
        self.softmax = FusedScaleMaskSoftmax(
            input_in_fp16=global_config.precision == "fp16",
            input_in_bf16=global_config.precision == "bfloat16",
            fusion_type=get_fusion_type(global_config),
            mask_func=mask_fn,
            softmax_in_fp32=global_config.attention_softmax_in_fp32,
            scale=None,
        )
...
class SpatialGatingUnit(nn.Module):
    def __init__(self, global_config, d_ff, d_attn=None, causal=True, mask_fn=None):
        super().__init__()
        self.causal = causal
        ...
        self.proj = nn.Linear(global_config.seq_length, global_config.seq_length)
        if self.use_attn:
            assert mask_fn is not None
            self.attn = TinyAttention(
                global_config=global_config, d_attn=d_attn, d_ff=d_ff, mask_fn=mask_fn
            )
...
class GMLPBlock(nn.Module):
    def __init__(
        ...
        ff_dim = global_config.hidden_size * ff_mult
        norm, eps = get_norm(global_config)
        self.norm = norm(global_config.hidden_size, eps=eps)
        ...
        if global_config.attention_config[layer_number] == "amlp":
            d_attn = global_config.gmlp_attn_dim
        else:
            d_attn = None
        self.sgu = SpatialGatingUnit(
            global_config, ff_dim_parallel, d_attn, causal=True, mask_fn=mask_fn
        )
