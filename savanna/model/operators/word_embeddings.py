...
from savanna.model.operators.positional_embeddings import SinusoidalPositionalEmbedding
...
class Embedding(torch.nn.Module):
    ...
    def __init__(
        ...
        self.seq_dim = self.global_config.seq_dim
        ...
        self.mup_embedding_mult = global_config.mup_embedding_mult
        self.mup_rp_embedding_mult = global_config.mup_rp_embedding_mult
        print_rank_0(f"DEBUG::EMBEDDING:: {vocab_size=} {self.hidden_size=}")
        ...
        if self.use_pos_emb:
            self.embedding_type = global_config.pos_emb
        ...
        self.opt_pos_emb_offset = global_config.opt_pos_emb_offset
    ...
    def forward(self, input_ids, position_ids, tokentype_ids=None):
        ...
        if self.global_config.checkpoint_embeddings:
            words_embeddings = checkpoint(self.word_embeddings, input_ids)
        else:
            words_embeddings = self.word_embeddings(input_ids)
#        print_rank_0(f"DEBUG::WORDS_EMBEDDINGS: {words_embeddings.shape} {words_embeddings.dtype}")
        ...
        if self.sequence_parallel:
            ...
            embeddings = mpu.scatter_to_sequence_parallel_region(embeddings, seq_dim=self.seq_dim)
...
class SoftEmbedding(torch.nn.Module):
    ...
    def initialize_embedding(self):
        if self.init_string:
            embeds = torch.LongTensor(self.global_config.tokenizer.tokenize(self.init_string)).to(
            ...
            return embeds
        return torch.Tensor(n_tokens, global_config.hidden_size).uniform_(
    ...
    def forward(self, args: tuple):
        ...
        if in_train:
            ...
            embedding = embedding[:, : self.global_config.seq_length, ...]
