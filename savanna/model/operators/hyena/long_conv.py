    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.weight = Parameter(
            torch.empty(1, 1, hidden_size_per_partition, device=ALLOC_DEVICE, dtype=global_config.params_dtype)
        )
...
class ParallelHyenaOperator(nn.Module):
    ...
    def __init__(
        ...
        world_size = mpu.get_model_parallel_world_size()
        self.hidden_size_per_partition = mpu.divide(global_config.hidden_size, world_size)

        self.L = global_config.seq_length
        self.num_heads = global_config.num_groups_hyena  # note we only cover hyena long here
