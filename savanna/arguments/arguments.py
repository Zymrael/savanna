...
class GlobalConfig(*BASE_CLASSES):
    ...
    def get_deepspeed_main_args(self):
        ...
        if "MASTER_ADDR" in os.environ and self.srun_launcher_type == "deepspeed":
            args_list.extend(
                self.convert_key_value_to_command_line_arg("master_addr", os.environ["MASTER_ADDR"])
            )

        if "MASTER_PORT" in os.environ and self.srun_launcher_type == "deepspeed":
            args_list.extend(
    ...
    def calculate_derived(self):
        ...
        global_num_gpus = getattr(self, "global_num_gpus", None)
        print(f"DEBUG::CALCULATE_DERIVED::global_num_gpus {global_num_gpus}")
        if global_num_gpus is None:
        ...
        assert all(
            [
                item in ["flash_te", "ring", "hyena", "hyena_medium_conv", "hyena_short_conv"]
        ...
        if self.is_context_parallel:
            if self.use_cp_flash_te:
                assert all(
                    [
                        item in ["flash_te", "hyena", "hyena_medium_conv", "hyena_short_conv"]
                        for item in self.operator_config
                    ]
                ), "CP requires use of flash_te, hyena, hyena_medium_conv, or hyena_short_conv operators"
            elif self.use_cp_ring:
                assert all(
                    [
                        item in ["ring", "hyena", "hyena_medium_conv", "hyena_short_conv"]
                        for item in self.operator_config
                    ]
                ), "CP requires use of ring, hyena, hyena_medium_conv, or hyena_short_conv operators"
            else:
                assert (
                    self.use_cp_hyena
                ), "CP requires use of hyena or CP attn operators (flash_te or ring), must set use_cp_hyena or one of use_cp_flash_te or use_cp_ring"
