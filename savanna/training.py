...
from savanna.debug import DEBUG_TENSOR_SAVE_DIR, save_tensor_hook
from savanna.initialize import initialize_megatron
from savanna.learning_rates import AnnealingLR
from savanna.logging import COMMS_LOG_RANKS
...
from savanna.memory_stats import maybe_enable_memory_snapshot, print_mem_alloc_stats
    ...
    if global_config.load is None or global_config.finetune:
        ...
        if global_config.iteration > 0 and not global_config.finetune:
            ...
            if not checkpoint_lr_scheduler:
                print_rank_0(f"LOAD_CHECKPOINT `checkpoint_lr_scheduler` is False even though resuming from checkpoint, setting to True")
            ...
            if not checkpoint_num_samples:
                print_rank_0(f"LOAD_CHECKPOINT `checkpoint_num_samples` is False even though resuming from checkpoint, setting to True")
...
def train(
    ...
    with maybe_enable_memory_snapshot(global_config, global_step=iteration) as memory_profiler:    
