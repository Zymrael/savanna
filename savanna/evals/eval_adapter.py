...
class EvalHarnessAdapter(GPT2LM):
    ...
    Args:
        ...
        global_config: a GlobalConfig object containing the model configuration.
        batch_size (optional): An argument to override the batch size, which defaults to batch size per gpu * dp world size.
    """

    def __init__(self, model, forward_step_fn, global_config, batch_size=None):
        self.cache_hook = base.CacheHook(None)
        self.model = model
        self.global_config = global_config
        self.tokenizer = global_config.tokenizer
        self._device = torch.device(f"cuda:{global_config.local_rank}")
        self._eot_token_id = global_config.tokenizer.eod_id
        self._max_length = global_config.seq_length  # max_position_embeddings // 2
        self._max_gen_toks = 128
        self._vocab_size = global_config.padded_vocab_size

        # parallelism args:
        self.is_main = global_config.rank == 0
        self.is_local_main = global_config.local_rank == 0
        self.is_model_parallel = global_config.model_parallel_size > 1
        ...
        self._batch_size = batch_size or (
            global_config.batch_size * self.dp_world_size
    ...
    def greedy_until(self, requests):
        ...
        for context, until in tqdm(reord.get_reordered(), "Running greedy generation"):
            ...
            cont = self.generate(
                ...
                recompute=self.global_config.recompute,
            )
            if cont:
                s = cont[0]["text"] or ""
            else:
    ...
    def run_eval(
        ...
        lm = self
        if use_cache:
            # TODO(jon-tow): Append a subset of `global_config` to the cache database
        ...
        results["config"] = {
            "model": name,
            "model_args": dataclasses.asdict(self.global_config),
            "num_fewshot": num_fewshot,
...
def run_eval_harness(
    ...
    adapter = EvalHarnessAdapter(
        model, forward_step_fn, global_config, batch_size=batch_size
    )
