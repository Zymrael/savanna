...
from savanna.utils import obtain_resource_pool, expand_attention_types
...
class GlobalConfig(*BASE_CLASSES):
    ...
    def __post_init__(self):
        ...
        if not GlobalConfig.validate_keys():
            raise ValueError(
                ...
                + ".__post_init__() GlobalConfig keys cannot be validated"
            )
        ...
        if not self.validate_types():
            raise ValueError(
                self.__class__.__name__
                + ".__post_init__() GlobalConfig types cannot be validated"
            )

        if not self.validate_values():
            raise ValueError(
                self.__class__.__name__
                + ".__post_init__() GlobalConfig values cannot be validated"
            )
    ...
    def consume_deepy_args(cls):
        ...
        if global_config.use_wandb:
            ...
            global_config.wandb_group += "_" + wandb.util.generate_id()
    ...
    def get_extra_deepspeed_args(self):
        ...
        """
        global_config = self.get_parent_class_value_dict(
    ...
    def get_deepspeed_main_args(self):
        ...
        encoded_mega_config = base64.urlsafe_b64encode(
            json.dumps(global_config).encode("utf-8")
        ).decode("utf-8")
