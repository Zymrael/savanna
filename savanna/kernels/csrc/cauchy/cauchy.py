...
def cauchy_mult_torch(v: torch.Tensor, z: torch.Tensor, w: torch.Tensor, symmetric=True) -> torch.Tensor:
    """
    v: (B, N)
    z: (L)
    w: (B, N)
    ...
    """
    if not symmetric:
        return (rearrange(v, "b n -> b 1 n") / (rearrange(z, "l -> l 1") - rearrange(w, "b n -> b 1 n"))).sum(
            dim=-1
        )
    else:
        N = v.shape[-1]
