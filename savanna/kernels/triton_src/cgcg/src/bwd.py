...
from savanna.kernels.triton_src.cgcg.src.kernel_utils import BwdKernelConfigRefactor
...
def two_pass_bwd_refactor(
    dy: torch.Tensor,
    # intermediate activations from fwd
    Bx: torch.Tensor,
    y2: torch.Tensor,  # convolution output (after correction) from fwd
    # input tensors
    x: torch.Tensor,
    B: torch.Tensor,
    C: torch.Tensor,
    ...
    fused=False,
    autotune=True,
    kernel_config: BwdKernelConfigRefactor = None,
):

    d = g * dg
    # Shape checks
    assert Bx.shape == y2.shape == x.shape == B.shape == C.shape == dy.shape
    assert dy.shape == (bs, seqlen, g, dg)
    ...
    if fused:
        ...
        # Pre-conv Kernel
        dy2 = torch.empty(bs, d, seqlen, dtype=dy.dtype, device=dy.device)
        dC = torch.empty_like(C)  # (bs, seqlen, d, dtype=dy.dtype, device=dy.device)
        Bx_permuted = torch.empty(bs, d, seqlen, dtype=dy.dtype, device=dy.device)

        grid = lambda META: (triton.cdiv(seqlen, META["BLOCK_X"]), triton.cdiv(bs * d, META["BLOCK_Y"]))

        if autotune:
            pre_conv_kernel_autotune[grid](dy, C, y2, Bx, dC, dy2, Bx_permuted, bs, seqlen, d)
        else:
            assert kernel_config is not None
            pre_conv_kernel[grid](
                dy,
                C,
                y2,
                Bx,
                ...
                seqlen,
                d,
                BLOCK_X=kernel_config.pre_conv_BLOCK_X,
                BLOCK_Y=kernel_config.pre_conv_BLOCK_Y,
                num_warps=kernel_config.pre_conv_num_warps,
            )

        # Backprop through Conv
        h_grouped = h.repeat_interleave(dg, dim=0)
        padding = filter_size - 1
        groups = d
        dBx = conv1d_input(Bx_permuted.shape, h_grouped, dy2, padding=padding, groups=groups)
        dh_grouped: torch.Tensor = conv1d_weight(
            Bx_permuted, h_grouped.shape, dy2, padding=padding, groups=groups
        )
        dh = dh_grouped.reshape(g, dg, 1, filter_size).sum(1)

        # Post Conv Kernel
        dx = torch.empty_like(x)
        dB = torch.empty_like(B)
        grid = lambda META: (triton.cdiv(bs * seqlen, META["BLOCK_X"]), triton.cdiv(d, META["BLOCK_Y"]))

        if autotune:
            post_conv_kernel_autotune[grid](dBx, x, B, dx, dB, bs, seqlen, d)
        else:
            assert kernel_config is not None
            post_conv_kernel[grid](
                ...
                seqlen,
                d,
                BLOCK_X=kernel_config.post_conv_BLOCK_X,
                BLOCK_Y=kernel_config.post_conv_BLOCK_Y,
                num_warps=kernel_config.post_conv_num_warps,
            )
    else:
        # Start backprop
        dC = dy * y2

        # dy2 is derivative wrt output of convolution
        dy2 = dy * C

        # reshape bx, dy2 from (bs, l, g, dg) -> (bs, g, dg, l) before passing to conv
        dy2 = dy2.permute(0, 2, 3, 1).reshape(bs, -1, seqlen)
        Bx_permuted = Bx.permute(0, 2, 3, 1).reshape(bs, -1, seqlen)

        # dg = group_dim = # of continguous channels sharing filter
        # expand filter to required shape for conv1d
        h_grouped = h.repeat_interleave(dg, dim=0)
        padding = filter_size - 1
        groups = d
        with torch.no_grad():
        ...
        # dh = dh_grouped.sum(dim=0, keepdim=True)

        # reshape back to (bs, l, g, dg)
        # dbx after conv is (bs, d, l)
        dBx = dBx.reshape(bs, g, dg, seqlen).permute(0, 3, 1, 2)
        dB = dBx * x
        dx = dBx * B

    # Output shape is (bs, l, g, dg)
    return dx, dB, dC, dh
