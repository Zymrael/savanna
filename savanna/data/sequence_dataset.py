...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.enforce_sample_length:
            for size in indexed_dataset.sizes:
                if size != seq_length:
                    raise ValueError(
                        f"Found sample with invalid length {size} (should be {seq_length}). "
                        f"Data at {data_prefix} needs to be reformatted."
                    )
        ...
        if build_index_mappings:
            ...
            self.doc_idx, self.sample_idx, self.shuffle_idx = _build_index_mappings(
                ...
                enforce_sample_length=self.enforce_sample_length,
            )
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.enforce_sample_length:
                assert doc_index_f == doc_index_l - 1
                assert offset_f == offset_l == 0
            ...
            return {"text": np.array(sample, dtype=self.sample_dtype)}
        except IndexError:
...
def _build_index_mappings(
    ...
    if should_process_dataset:
        if (
            ...
            if 2.0 * (num_samples + 1) < np.iinfo(np.int32).max:
    ...
    assert counts[0].item() == torch.distributed.get_world_size(group=mpu.get_io_parallel_group())
...
def _build_doc_idx(documents, num_epochs, np_rng):
    ...
    dtype_ = np.uint32
    if 2.0 * (len(doc_idx) + 1) >= (np.iinfo(np.uint32).max - 1):
        dtype_ = np.int64
    doc_idx = doc_idx.astype(dtype_)
...
def _build_shuffle_idx(size, np_rng):
    ...
    dtype_ = np.uint32
    if (2.0 * size) >= (np.iinfo(np.uint32).max - 1):
        dtype_ = np.int64
