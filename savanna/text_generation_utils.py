...
from savanna.utils import get_ltor_masks_and_position_ids, is_mp_rank_0


def get_batch(global_config, tokenizer, context_tokens: torch.Tensor):
    """
...
def pad_batch(context_tokens: List[List[int]], pad_id: int, pad_len: int):
    """
    pads context lengths in context_tokens with pad_id to equal global_config.seq_length,
...
def stream_tokens(
    ...
    context_tokens, context_lengths = pad_batch(
        ...
        pad_len=global_config.seq_length,
    )
    ...
    maximum_tokens = maximum_tokens or (
        global_config.seq_length - token_generation_start_index.max().item() - 1
    )
...
def generate_samples_from_prompt(
    ...
    while True:
        ...
        if input_pos == input_count:
            ...
            if context_length >= (global_config.seq_length // 2):
                print_rank_0(
...
def generate_samples_input_from_file(
    ...
    Reads prompts from global_config.sample_input_file and writes completions to global_config.sample_output_file
