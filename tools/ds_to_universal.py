...
"""
7/19/24, Eric N.
This is a frozen copy of the DeepSpeed script to convert a DeepSpeed checkpoint to a Universal checkpoint.


To use deepspeed conversion correcty, we need to make a small correction in the DeepSpeed code:

- DeepSpeed version 0.14.4 (only one that's been tested)
- Make a change in one of the files here:
    https://github.com/microsoft/DeepSpeed/blob/78c6c449c967e0c0f4755fc8256376b177ef84ef/deepspeed/checkpoint/reshape_3d_utils.py#L76
    
    # location will likely be:
    /path/to/deepspeed/deepspeed/checkpoint/reshape_3d_utils.py
    
    # replace line with:
    num_pp0_files = len(get_files_with_prefix(file_list, f'{LAYER_FILE_PREFIX}02'))  # change 01 to 02

    # reason:
    The code counts the number of repeated layers (in the checkpoint dir) to find the model parallel degree, but savanna skips layer_01, so we use any other one (layer_02) instead.

- Remove this assert as well:
    https://github.com/microsoft/DeepSpeed/blob/afe1b9ede1794b0bf221607be8051728cf4afa18/deepspeed/checkpoint/deepspeed_checkpoint.py#L302

Example usage for converting:

python ds_to_universal.py \
    --input_folder /checkpoint/hielab/etnguyen/checkpoints/evo2/7b_15h_7m_7sc_3a_400k/global_step9000 \
    --output_folder /checkpoint/hielab/etnguyen/checkpoints/evo2/7b_15h_7m_7sc_3a_400k/global_step9000_universal \
    --inject_missing_state

--input_folder: path to the DeepSpeed checkpoint (global step) folder, from regular Savanna code.
--output_folder: output folder for the Universal checkpoint, make sure to follow the format of appending "_universal" after the original global step folder name.
    Needed for Savanna to correctly load the checkpoint.
--inject_missing_state flag: needed to create a state file that DS looks for.



Changes in Savanna config needed to resume training from the converted universal checkpoint:

Add these flags in config:

"checkpoint": {"load_universal": True},
"iteration": 4,  # need this to automatically find the path. "4" is an example
"load": "/checkpoint/hielab/etnguyen/checkpoints/evo2/100m_10k_4h_4sc_4m_2a_524k_save_ds_test_new1/",  # original non-universal checkpoint path
"use_checkpoint_lr_scheduler": True,


Note the format for the load path.  We follow the convention of using the original checkpoint path, and then Savanna will properly append the iteration number 
and the "_universal" tag to find the checkpoint.
...
def extract_zero_shards_stage3(optim_files, param_shapes, dp_degree, temp_dir, dp_index):
    ...
    for name, shape in param_shapes.items():
        ...
        for state_key in flat_state.keys():
            dump_param_fragment(
                temp_dir, 0, dp_index, state_key, flat_state[state_key], name, offset, padding_free_numel
            )
...
def merge_tp_slices(ds_checkpoint, dir, slice_dir, tp_degree, name_and_shape):
    ...
    unmatched_patterns.update(
        chain.from_iterable(SubparamShape(**s).patterns for s in parameter_with_sub_params)
    )
    ...
    for state in ("fp32", "exp_avg", "exp_avg_sq"):
        ...
        if get_matched_pattern(replicated_parameters, name):
            ...
            partition_shape = [
                d // tp_degree if i == partition_dim else d for i, d in enumerate(partition_shape)
            ]
            ...
            for sub_dim_size in sub_dim_sizes:
                ...
                merged_chunks.append(
                    torch.cat(
                        [s.narrow(partition_dim, offset, part_sub_dim_size) for s in slices],
                        dim=partition_dim,
                    )
                )
...
def _extract_zero_shard_files(args, ds_checkpoint, temp_dir):
    _3d_range_list = list(
        itertools.product(
            range(ds_checkpoint.pp_degree), range(ds_checkpoint.tp_degree), range(ds_checkpoint.dp_degree)
        )
    )
